// based on readability by arc90: http://code.google.com/p/arc90labs-readability
// removed lots of extraneous stuff that we didn't need
// added ability to copy a specific node, and add it to a specific node 
// (rather than reading the whole body)
...
var readability = {
    ...
    init: function(sourceElement, overlayID, parentID) {		
		...
		// in the future, we may want to prep the document. the main thing here is this strips out iframes,
		// and converts double BRs to P
    ...
    **/
     ...
     * @return Element
    **/
    grabArticle: function (element) {
        var stripUnlikelyCandidates = readability.flagIsActive(readability.FLAG_STRIP_UNLIKELYS);
        var isPaging = false;

		var clone = $(element).clone()[0];
		$(clone).addClass('readabilityTemp');
		document.body.appendChild(clone);
		
		var allElements = clone.getElementsByTagName('*'); // changed from page to work on clone
        ...
        **/
		...
		// SK: this should be fine as long as we don't run readability on the raw body
		if (topCandidate === null) {
			topCandidate = clone;
		}
		if (topCandidate.tagName === "BODY") {
			topCandidate = $(topCandidate).wrap('<div>');
		}
