...
carnival.user = function($,window,document,undefined){
	...
	var setCarnivalCookie = function(cancelCallback) {
		if(!config('cookieTime')){
			...
			sixMonthDate.setTime(date.getTime()+(180*24*60*60*1000));
		//	rmcDate.setTime(rmcDate.getTime()+(59*24*60*60*1000));
			config('cookieTime',date);
			config('sixMonthCookieTime',sixMonthDate);
		...
		// 	dropLoginCookies();
		// 	//carnival.utils.cookies.set(config('c_rmc'), 't', {expiresAt: config('sixMonthCookieTime'),domain:config('cookieDomain')});
		// 	if(!cancelCallback){carnival.utils.listener.fire('_carnival_after_login');}
		// 	try{carnival.utils.listener.fire('_carnival_after_cookie');}catch(e){}
		// } else {
		// 	
	...
	var dropLoginCookies = function() {
		carnival.utils.cookies.set(config('c_mid'), profile('masterId'), {expiresAt: config('sixMonthCookieTime'),domain:config('cookieDomain')});
				carnival.utils.cookies.set(config('c_unm'), profile('userName'), {expiresAt: config('sixMonthCookieTime'),domain:config('cookieDomain')});
